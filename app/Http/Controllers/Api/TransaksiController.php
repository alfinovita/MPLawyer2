...
class TransaksiController extends Controller{
    ...
    public function allTagihan(Request $request)
    {
        if($request->user->role == 'LAWYER'){
            $pembayaran = Pembayaran::select('id', 'type', 'status', 'amount', 'created_at as created', 'detail_id')->where('lawyer_id', $request->user->id)->orderBy('id', 'desc')->get();
        }else{
        ...
        foreach ($pembayaran as $k => $m) {
            if($m->type == 'LIVE_CHAT'){
                ...
                $id = ($request->user->role == 'LAWYER')?$data->client_id:$data->lawyer_id;
                $user = User::find($id);
    ...
    public function callback(Request $request)
    {
        ...
        if($request->status == 'PAID' || $request->status == 'SETTLED'){
            ...
            if($pembayaran->type == 'LIVE_CHAT'){
                ...
                $oneYearOn = date('Y-m-d',strtotime(date("Y-m-d", time()) . " + ".$waktu = Setting::find(3)->value." day"));
                $pengajuan->update([
                    'status' => 'PAID',
