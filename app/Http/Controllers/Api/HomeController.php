...
class HomeController extends Controller{
    ...
    {
        $id = $request->bidang_id;
        $notaris = User::select('id', 'avatar', 'nama_lengkap')->where('role', 'NOTARIS')->get();
        if($notaris){
    ...
    public function detailLawyer(Request $request)
    {
        ...
        $review = DB::table('konsultasi')
        ->select('users.id as client_id', 'nama_lengkap', 'avatar', 'konsultasi.rating', 'konsultasi.review')
        ->join('users', 'konsultasi.client_id', '=', 'users.id')
        ->where('konsultasi.lawyer_id', $lawyer_id)
        ->where('konsultasi.service_id', $service_id)
        ->where('konsultasi.status', 'FINISH')
        ->get();
        $lawyer = DB::table('users')
        ...
        ->first();
        return response()->json(['message' => 'Data lawyer detail','code' => 1, 'error' => false, 'status_konsultasi' => $status, 'konsultasi_id' => $id , 'status_expired' => $expired, 'rating' => 5, 'lawyer_detail' => $lawyer, 'review' => $review]);
    }
