...
class LayananController extends Controller{
    ...
    public function getChat(Request $request)
    {
        ...
        return response()->json(['message' => 'Data Chat', 'code' => 1, 'error' => false, 'id' =>  $request->id,  'chat' => $chat1,'id_lawan' => $user_lawan->id, 'avatar_lawan' => $user_lawan->avatar, 'nama_lawan' => $user_lawan->nama_lengkap, 'harga_vicon' => $service->harga_vicon??'', 'harga_live_chat' => $service->harga??'', 'layanan' => $konsultasi->service->nama??'Probono']);
    }
    ...
    public function finishKonsultasi(Request $request)
    {
        ...
        if($data){
            if($data->status == 'ON_GOING' || $data->status == 'PAID'){
                if($request->user->role == 'LAWYER'){
                    if($data->lawyer_id != $request->user->id){
                        return response()->json(['message' => 'Konsultasi tidak ditemukan', 'code' => 0, 'error' => true]);
                    }
    ...
    {   
        ...
        if($data){
            if($data->status == 'FINISH_LAWYER'){
                ...
                return response()->json(['message' => 'Konfirmasi berhasil dibatalkan', 'code' => 1, 'error' => false]);
            }else{
    ...
    public function getListReport(Request $request)
    {
        if($request->user->role == 'LAWYER'){
            ...
            ->select('report.id', 'report.judul_report', 'report.created_at', 'report.penjelasan')
